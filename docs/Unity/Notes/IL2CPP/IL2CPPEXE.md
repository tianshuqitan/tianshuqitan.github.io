...
[进程实用工具](https://learn.microsoft.com/zh-cn/sysinternals/downloads/process-utilities)

**IL2CPP 的 AOT 编译器**，由 C# 编写而成，位于 `Unity安装目录/Editor/Data/il2cpp/build/deploy` 下。`il2cpp.exe` 工具接收由 Unity 附带的 Mono 编译器编译的托管程序集，并生成 C++ 代码，随后我们将这些代码交由特定平台的 C++ 编译器处理。
